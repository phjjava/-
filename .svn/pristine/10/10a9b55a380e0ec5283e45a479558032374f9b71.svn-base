...
<article class="page-container">
   <form class="form form-horizontal" enctype="multipart/form-data" method="post" id="photo-add">
	 <input type="hidden" id="albumid" name="albumid" value="${albumid}"/>
	 <input type="hidden" id="userid" name="userid" value="${userid}"/>
	 <input type="hidden" id="type" name="type" value="${type}"/>
	 <div class="row cl" style="color:#333333;">
        <c:if test="${type == 0}">
			<label class="form-label col-xs-4 col-sm-2" >图片上传：</label>
		</c:if>
		<c:if test="${type == 1}">
			<label class="form-label col-xs-4 col-sm-2" >作品上传：</label>
		</c:if>
		<div class="formControls col-xs-8 col-sm-9">
			<div class="uploader-list-container"> 
				<div class="queueList">
					<div id="dndArea" class="placeholder">
						...
						<p>或将图片拖到这里, 批量上传最多不超过200M, 单张不能超过50M, 图片格式为jpg或png</p>
					</div>
				</div>
			...
			<div class="row cl" style="text-align:center">
				<div class="col-xs-8 col-sm-9 col-xs-4 col-sm-2" style="width:100%;" >
					<input class="btn btn-success radius" type="button" onclick="confirm();" value="确认" style="width:120px;">
				</div>
		    </div>
		</div>
	</div>
...
(function( $ ){
	    ...
	    $(function() {
	        ...
	        uploader = WebUploader.create({
	            ...
	            chunked: false,
	            chunkSize: 512 * 1024,
	            server: '<%=basePath%>user/saveAlbum',
