...
public interface SysVersionPrivilegeMapper {
	...
	@Delete("DELETE FROM jp_sys_version_privilege WHERE versionid = #{versionid} ")
	int deleteByVersionid(@Param("versionid") String versionid);

	SysVersionPrivilege selectVersionValue(@Param("versionid") String versionid,
	 ...
	 * @return
	 */
	SysVersionPrivilege selectByVersionAndCode(@Param("familyid") String familyid,
			@Param("privilegecode") String privilegecode);
