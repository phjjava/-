...
public class ExcelUtil {
	...
	public void showExcelFile(String excelName, List<?> resultList, List<?> listKey, List<?> listName, String beanName)
	...
	// 将信息加入excel表格中
	public void insertInfo(HSSFSheet sheet, HSSFCellStyle styleContent, HSSFCellStyle styleFomatDouble,
			HSSFDataFormat format, List<?> vos, List<?> listKey, String beanName) {
		int k = 0;
		//列
		HSSFRow row = null;
		//行
				...
				for (int keyCount = 0; keyCount < listKey.size(); keyCount++) {
					...
					try {
						...
						if (obj.toString().equals("int") || obj.toString() == "int") {
							cell.setCellValue((double) Integer.parseInt(tempObj == null ? ""
									: o.getClass().getField(listKey.get(keyCount).toString()).get(o).toString()));
						} else if (obj.toString().equals("double") || obj.toString() == "double") {
							cell.setCellValue(Double.parseDouble(tempObj == null ? ""
									: o.getClass().getField(listKey.get(keyCount).toString()).get(o).toString()));
						} else if (obj.toString().equals("class java.lang.Double")
								|| obj.toString() == "class java.lang.Double") {
							if (tempObj == null) {
								s = 0.00;
							} else {
