...
public class UploadUtil {
	...
	public static String fileUpload(List<File> files, List<String> fileNames) throws FileNotFoundException {
		...
		try {
			...
			// Part[] parts = { new FilePart(targetFile.getName(), targetFile),new
			// FilePart(targetFile1.getName(), targetFile1) };
			...
			ex.printStackTrace();
			result = "{\"result\": 1," + "\"title\": \"文件上传\"," + "\"msg\": \"上传失败\"," + "\"data\": {" + "}}";
		} finally {
	...
	public static String webFileUpload(List<File> files, List<String> fileNames, String url, String companyid,
			String createid, String createname) throws FileNotFoundException, UnsupportedEncodingException {
	 ...
	 */
	public static String groupFileUpload(List<File> files, List<String> fileNames, String loginId, String groupid)
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public static String taskFileUpload(List<File> files, List<String> fileNames)
			throws FileNotFoundException, UnsupportedEncodingException {
		// Part[] parts = new Part[files.size()];
		String filename = "";
		// 文件
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public static Map<String, Object> batchFileUpload(List<File> files, List<String> fileNames)
			throws FileNotFoundException, UnsupportedEncodingException {
		// Part[] parts = new Part[files.size()];
		String filename = "";
		// 文件
		...
		Map<String, Object> gsonToMaps = null;
		for (int i = 0; i < files.size(); i++) {
			File file = files.get(i);
			filename = fileNames.get(i);
			filePart = new CustomFilePart(filename, file);
			...
			try {
				...
				if (status == HttpStatus.SC_OK) {
					...
					result = new String(filePost.getResponseBodyAsString().getBytes("utf-8"));
					gsonToMaps = GsonUtil.GsonToMaps(result);
					res.add(gsonToMaps.get("data"));
					// String result1= new String(result.getBytes("ISO8859_1"),"utf-8");
					// System.out.println(result1);
					System.out.println(result);
					// 上传成功
