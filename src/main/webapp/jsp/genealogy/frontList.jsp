...
<%
	...
	String showUrl = ConstantUtils.JIAPU_SHOW_URL;
%>
...
<nav class="breadcrumb"><i class="Hui-iconfont"></i> <a href="/" class="maincolor">首页</a> 
	...
	<span class="c-999 en">&gt;</span>
	<span class="c-666">封面设置</span> 
</nav>
<link rel="stylesheet" type="text/css" href="<%=basePath%>lib/webuploader/0.1.5/webuploader.css"  />
<div class="Hui-article">
  <article class="cl pd-20">
	<div class="mt-20">
	<form class="form form-horizontal" id="cover_add">
	    <input type="hidden" id="instructionid" name="instructionid" value="${instruction.instructionid}"/>
		<input type="text" hidden class="input-text"  value="${instruction.imgurl}" id="coverUrl" name="imgurl">
		<input type="text" hidden  class="input-text"  value="${instruction.imgurl}" id="bigCoverUrl" name="bigImgurl">
		<div class="row cl" id="showImg">
			<label class="form-label col-xs-4 col-sm-2">图片预览：</label>
			<div class="formControls col-xs-8 col-sm-5" style="border:1px solid #dddddd;min-height:200px;height:auto;text-align:center;padding:20px">
				<img src="" id="preImg" hidden style="width:190px;height:150px;background-color:#f8f8f8;margin:15px 7px 0 7px;"/>
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">图片上传：</label>
			<div class="formControls col-xs-8 col-sm-5" style="padding:0">
				<div class="uploader-list-container"> 
					<div class="queueList">
						<div id="dndArea" class="placeholder">
							<div id="filePicker-2"></div>
							<p>仅限制上传一张，图片格式为jpg或png</p>
						</div>
					</div>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<a onclick="cover_save();" class="btn btn-primary radius"><i class="Hui-iconfont">&#xe632;</i> 保存</a>
			</div>
		</div>
	</form>
	</div>
...
//预览
function preShowIMG(){
	var pUrl=$("#coverUrl").val();
	if(pUrl!=null&&pUrl!=''){
		var imgSRC=imgPreUrl+pUrl+"?curSec="+Math.random();
...
function cover_save(){
		$.ajax({
			...
			dataType:'json',
			async: true,
			data:$("#cover_add").serialize(),
			url : '<%=basePath%>instruction/saveCover?curSec='+Math.random(),
			success:function(data){
				var rtdata=data.status;
				console.warn(data);
				if(rtdata == '1'){
            ...
            if(ret != null && ret != '') {
                ...
                if(data != null && data != '') {
                    ...
                    if(data == '0') {
                    	...
                    	$("#coverUrl").val(data.iconUrl);
                    	$("#bigCoverUrl").val(data.url);
                    	//alert(data.iconUrl);
                        flag = true;
                        preShowIMG();
                    }
