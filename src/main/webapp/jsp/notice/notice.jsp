...
<article class="page-container">
  <form action="" class="form form-horizontal" method="post" id="notice-add">
  	<code id="testcon" style="display:none;">
        ${notice.noticecontent }
    </code>
    <input type="hidden" id="branchname" name="branchname" value="${notice.branchname}"/>
    ...
    <input type="hidden" id="fileids" name="fileids" >
	  <div class="row cl">
		...
		<div class="formControls col-xs-8 col-sm-9">
			<input type="text" class="input-text" value="${notice.noticetitle}"  name="noticetitle">
	   </div>
     </div>
     <div class="row cl" id="notice-type">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>选择公告类型：</label>
			<div class="formControls col-xs-8 col-sm-9" >
			    <div>
				    <select id="noticetype" class="input-text select" name="noticetype" data-val="${notice.noticetype }">
				    	<option value="0" <c:if test="${notice.noticetype == 0 }">selected = "selected"</c:if>>家族公告</option>
				    	<option value="1" <c:if test="${notice.noticetype == 1 }">selected = "selected"</c:if>>分支公告</option>
			        </select>
		        </div>
		    </div>
		</div>
     <div class="row cl" id="branch-select">
	   <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>所属分支：</label>
	    <div class="formControls col-xs-8 col-sm-9">
		   <select name="branchid" id="branchid" class="branch-select select" data-val="${notice.branchid }" >
		   </select>
	    </div>
      </div>
      <div class="row cl">
		    ...
		    <div class="formControls col-xs-8 col-sm-9">
			   <select data-placeholder="---- 请选择置顶分支 ----" name="tobranchid" id="tobranchid" multiple class="multiple-branch-select select" data-val="${notice.tobranchid }" class="branch-select select"></select>
		    </div>
      </div> 
	  <div class="row cl">
	...
	<c:forEach var="notice" items="${ntlist}" varStatus="status">
		<div class="row cl" >
			...
			<div class="formControls col-xs-8 col-sm-8">
			    ...
			    <a href="javascript:;" id="deletentf" onclick="deleteSingleFile(this,'${notice.fileid}');" class="btn btn-danger radius" style="margin-left:50px;">
	...
	<div class="row cl">
		<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2" style="text-align:right">
			<input class="btn btn-primary radius" type="button" onclick="noticeSave();" value="确认" style="width:120px;">
			<input class="btn btn-primary radius" type="button" onclick="layer_close();" value="取消" style="width:120px;">
		</div>
	</div>
  </form>
</article>
...
//var fileServer='http://fileupload.aojiapu.com/jsp/controller.jsp?action=uploadimage&encode=utf-8';
  ...
  function noticeSave(){
      ...
      if(branchid == null||branchid==''){
    	  layer.alert("请选择分支", {icon: 7});
    	  return false;
      }
      if(noticecontent == null||noticecontent==''){
    	  layer.alert("请填写通知内容", {icon: 7});
    	  return false;
      }
	  ...
	  $.ajax({
		 ...
		 async: true,
		 data:formData,
		 url : '<%=basePath%>notice/save?curSec='+Math.random(),
		 processData : false, 
		 contentType : false,
		 success:function(data,status){
			 if(status=="success"&&data=="1"){
				...
				window.parent.layer.msg('保存失败!',{icon:6,time:1000});
			 }
		 },
		 error:function(e){
			 console.log(e);
  ...
  $(function(){
		...
		initBranch();
			...
			success:function(data,status){
				if(data){
					//var optionStrM1 = "<option value=''>---- 请选择分支----</option>";
					var optionStrM1 = "";
