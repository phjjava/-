...
<div class="Hui-article">
	<article class="cl pd-30">
		<div class="cl pd-5 bg-1 bk-gray">
			<span class="l"> 
			  <a href="javascript:;" onclick="userrolebranch_add('添加管理员','<%=basePath%>jsp/management/userrolebranch.jsp','','800','450')" class="btn btn-primary radius">
			    <i class="Hui-iconfont">&#xe600;</i> 添加管理员
			  </a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			</span>
			<span class="inline">
				<input type="text"  id="username" class="input-text" style="width:200px;margin-left:20px;padding-left:10px;" value="${userrole.username}" placeholder="请输入姓名"/>
...
<%-- 									<c:if test="${user.deleteflag == 1}">
									...
									<c:choose>
									<c:when test="${user.ismanager == 1 && user.type == 1}">
										<span style="color:red">家族管理员<span>		
									</c:when>
									<c:otherwise>
										<a title="编辑" href="javascript:;" onclick="userrolebranch_edit('编辑成员','${user.userid}','','800','450')" onclick="userrolebranch_edit('管理员编辑','userrole/userrolebranch.jsp','1')" class="ml-5" style="text-decoration:none;color:#5a98de">编辑</a>&nbsp;&nbsp;|&nbsp;&nbsp;
								
										 <a style="text-decoration:none;color:#5a98de" onClick="userrolebranch_delete('${user.userid}','${user.roleid}','${user.familyid}')" href="javascript:;" title="停用">删除</a>
									</c:otherwise>
								    </c:choose>
								</td>
							</tr>
...
/* /*管理员-编辑*/
/* function userrolebranch_edit(title,url,id,w,h){
	layer_show(title,url,w,h);
} */ 
/*编辑
*/
function userrolebranch_edit(title,userid,id,w,h){
	var url = basePath + 'userrole/edit?userid='+userid+'&curSec='+Math.random();
	layer_show(title,url,w,h);
}
/*管理员-删除*/
function userrolebranch_delete(id1,id2,id3){
	var userid=id1;
	var roleid=id2;
	var familyid=id3;
	layer.confirm('确认要删除吗？删除后如需恢复请重新添加',function(index){
		$.ajax({
			type: 'POST',
			data:"userid="+userid+"&roleid="+roleid+"&familyid="+familyid,
			url: basePath + 'userrole/deleteUserRole?curSec=' + Math.random(),
			dataType: 'json',
...
function searchs(pageNoTemp) {
	...
	$.ajax({
		...
		url : basePath + 'userrole/list?curSec=' + Math.random(),
		//async : false,
		success : function(data) {
