...
<div class="Hui-article">
	<article class="cl pd-30">
		<div class="cl pd-5 bg-1 bk-gray">
		  <span class="l">
		    <a href="javascript:;" onclick="role_add('新增职务','<%=basePath %>role/get','800','800')" class="btn btn-primary radius">
		...
		<div class="mt-10">
			<div id="DataTables_Table_0_wrapper" class="dataTables_wrapper no-footer">
				<table class="table table-border table-bordered table-bg" style="margin-top:5px;">
					...
					<tbody>
					   <c:forEach var="role" items="${pageModel.list}" varStatus="status">
						<tr class="text-c">
							<td>${status.index + 1 }</td>
							<td>${role.rolename}</td>
							<td>${role.roledesc }</td>
							<td class="td-manage">
							  <a href="javascript:;" onclick="role_edit('编辑成员','${role.roleid}','800','800')">编辑</a>&nbsp;|&nbsp;
							  <a href="javascript:;" onclick="role_del('${role.roleid}')">删除</a>
							</td>
						</tr>
					  </c:forEach>
					</tbody>
...
function searchs(pageNoTemp) {
	...
	$.ajax({
		...
		url : basePath + 'role/list?curSec=' + Math.random(),
		//async : false,
...
<script type="text/javascript">
/*管理员-增加*/
function role_add(title,roleid,id,w,h){
	var url = basePath + 'role/get?roleid='+roleid+'&curSec='+Math.random();
	layer_show(title,url,w,h);
}
/*管理员-编辑*/
function role_edit(title,roleid,id,w,h){
	var url = basePath + 'role/get?roleid='+roleid+'&curSec='+Math.random();
	layer_show(title,url,w,h);
}
/*管理员-删除*/
function role_del(roleid){
	layer.confirm('确认要删除吗？',function(index){
		$.ajax({
			type: 'post',
			data: "roleid="+roleid,
			url: basePath + 'role/del?&curSec='+Math.random(),
			dataType: 'json',
			success: function(data){
				if(data.data==1){
